# Khamim Portfolio — Renaissance Gallery on Putty Paper

Portfolio pribadi [Khamim Zarkasyi](https://github.com/khamimzarr) — warm putty `#c4c3b6`, ink `#000000`, Davinci 374px wordmark, flat, no shadow.

**Live:** https://khamim-portfolio.vercel.app

## Stack
- Next.js 16 (App Router) + Tailwind 4
- Playfair Display (Davinci) + Inter (Helvetica Now)
- Vercel — project `khamim-portfolio` (team `voiddarkfire`)

## Hobi / Exhibition
Foto hal random, bersepeda, merawat tanaman, ikan hias & aquarium, vibes coding, ngopi + nge-project — dikurasi sebagai 6 Circular Vignettes.

## Karya
- hr-ai-review — https://hr-ai-review-nine.vercel.app
- khamim-portfolio — https://khamim-portfolio.vercel.app
- Clouds — https://clouds-chi.vercel.app
- dompetkua-pp — https://dompetkua-pp.vercel.app

```bash
npm install
npm run build
npm run dev
```

## Handoff
Lihat [HANDOFF.md](./HANDOFF.md) untuk lanjut Tier 2.
